...
Removing the plugin from **Setup → Plugins** drops all RoundRobin database tables (`rr_assignments`, `rr_options`, `rr_groups`). Category toggles and rotation state are removed. Reinstalling starts with a fresh configuration.

Log files under GLPI’s `files/_log/` (for example `roundrobin.log`) are not deleted automatically.
